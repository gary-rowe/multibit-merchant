...
public class Item implements Serializable {
   ...
   */
  @Transient
  public String getItemFieldContent(ItemField itemField, Locale locale) {

    // Get the field detail that contains the primary and secondary content
    ItemFieldDetail itemFieldDetail = itemFieldMap.get(itemField);

    if (itemFieldDetail != null && locale != null) {
      // Attempt to locate the content for the given locale

      //String localeKey = String.format("%s_%s_%s",locale.getLanguage(),locale.getCountry(),locale.getVariant());
   ...
   */
  @Transient
  public ItemFieldDetail getItemFieldDetail(ItemField itemField, Locale locale) {
    if (locale == null) {
